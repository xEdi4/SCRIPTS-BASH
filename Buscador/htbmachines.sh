...
function updateFiles(){
  if [ ! -f bundle.js ]; then
    ...
    if [ "$md5_temp_value" == "$md5_original_value" ]; then
      echo -e "\n${yellowColour}[+]${endColour}${grayColour} No se han detectado actualizacines, lo tienes todo al día ;)${endColour}\n"
      rm bundle_temp.js
    else
...
function getMachinesDifficulty(){
  ...
  if [ "$results_check" ]; then
    ...
    cat bundle.js | grep "dificultad: \"$difficulty\"" -B 5 | grep "name: " | awk 'NF{print $NF}' | tr -d '"' | tr -d ',' | column && echo ""
  else
...
function getOSMachines(){
  ...
  if [ "$os_results" ]; then
    ...
    cat bundle.js | grep "so: \"$os\"" -B 5 | grep "name: " | awk 'NF{print $NF}' | tr -d '"' | tr -d ',' | column && echo ""
  else
...
function getOSDifficultyMachines(){
  ...
  check_results="$(cat bundle.js | grep "so: \"$os\"" -C 4 | grep "dificultad: \"$difficulty\"" -B 5 | grep "name: " | awk 'NF{print $NF}' | tr -d '",' | column)"
  ...
  if [ "$check_results" ]; then
    ...
    cat bundle.js | grep "so: \"$os\"" -C 4 | grep "dificultad: \"$difficulty\"" -B 5 | grep "name: " | awk 'NF{print $NF}' | tr -d '",' | column && echo ""
  else
...
function getSkill(){
  ...
  if [ "$check_skill" ]; then
    ...
    cat bundle.js | grep "skills: " -B 6 | grep "$skill" -i -B 6 | grep "name: " | awk 'NF{print $NF}' | tr -d '",' | column && echo ""
  else
